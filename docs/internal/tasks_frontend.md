...
### Задача 18: Адаптация CSS под темы Telegram (US-10)
...
**Цель:** Оживить интерфейс приложения, добавив вибрационный отклик на ключевые действия пользователя.

### Задача 19: Интеграция Haptic Feedback через Telegram SDK
...
**Цель:** Интегрировать нативную кнопку «Назад» Telegram и защитить пользователя от случайного закрытия формы с несохраненными данными.

### Задача 20: Нативная навигация (BackButton) и защита от закрытия (ClosingConfirmation)
...
* **Описание:**
  1. **BackButton:**
     * В `js/telegram.js` создать методы `showBackButton(onClick)` и `hideBackButton()`.
...
**Цель:** Реализовать возможность редактировать созданные челленджи через UI.

### Задача 21: Метод API и обновление Store
* **Файлы:** `frontend/js/api.js`, `frontend/js/store.js`
* **Описание:**
...
### Задача 22: UI Формы редактирования
* **Файлы:** `frontend/js/components/challenge/challenge-detail.js`, `frontend/js/components/challenge/challenge-form.js`
* **Описание:**
...
* **Ограничения:**
  * Переиспользовать существующий компонент формы, чтобы избежать дублирования кода.

---

## Epic: US-14 Ачивки внутри челленджа

**Цель:** Реализовать отображение пула из 8 ачивок на странице деталей челленджа с информационными попапами.

### Задача 23: Адаптация API Client и Store для ачивок челленджа
* **Файлы:** `frontend/js/api.js`, `frontend/js/store.js`
* **Описание:**
  1. Изменить метод в API клиенте на `getChallengeAchievements(challengeId)` (`GET /api/challenges/${challengeId}/achievements`). Убрать старый `getAchievements()`.
  2. В Store при установке текущего челленджа (`setCurrentChallenge`) и при инициализации загружать ачивки именно для этого челленджа. Хранить их в `state.currentChallengeAchievements`.

### Задача 24: Отображение 8 ачивок на странице деталей
* **Файлы:** `frontend/js/components/challenge/challenge-detail.js`
* **Описание:**
  1. В методе `render` добавить рендеринг секции с 8 иконками ачивок (маппинг всех 8 ачивок хардкодом с их названиями и иконками).
  2. Если ачивка есть в массиве `currentChallengeAchievements`, отображать её яркой (opacity 1). Иначе — полупрозрачной с ч/б фильтром.

### Задача 25: Информационные попапы ачивок
* **Файлы:** `frontend/js/components/ui/achievement-popup.js`, `challenge-detail.js`
* **Описание:**
  1. В `achievement-popup.js` добавить метод `showAchievementInfo(code)`, который показывает информацию по одной конкретной ачивке (описание, как её получить).
  2. В `challenge-detail.js` добавить слушатели клика на все 8 иконок. При клике показывать этот попап.
  3. Сделать автозакрытие попапа через 5 секунд или по клику на "крестик"/оверлей.

### Задача 26: Удаление старого блока ачивок с Дашборда
* **Файлы:** `frontend/js/components/dashboard/dashboard.js`
* **Описание:**
  Удалить статичный блок "Мои достижения" с главного экрана, так как ачивки теперь живут только внутри деталей челленджа.
