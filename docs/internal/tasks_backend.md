...
### Задача 19: Откат / удаление ачивок при удалении тренировок
* **Файлы:** `internal/database/workout.go`, `internal/database/achievement.go`
* **Описание:**
  1. При удалении тренировки (`DELETE /api/workouts/:id`), если `current_progress` опускается ниже порогов (50%, 100%, 120%), нужно удалять (revoking) соответствующие разблокированные достижения (`equator`, `hero`, `overachiever`, `stability`, `first_step` и т.д.), условия которых больше не выполняются.
  2. Разработать алгоритм автоматического пересчета/проверки актуальности достижений при удалении тренировки и каскадно очищать таблицу `user_achievements` для данного челленджа.
